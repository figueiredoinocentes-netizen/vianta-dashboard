...
- **2 deploys de `master` falharam hoje** por volta das 15:50–15:51 UTC, no commit `272ae23` ("add CLAUDE.md"), erro `Build script returned non-zero exit code: 2`. Não relacionado com este PR — não investiguei porque parecia estar a acontecer em paralelo (talvez o próprio Hermes a testar). Precisa de diagnóstico (suspeita: `postbuild.py` a falhar por dependência do `python3` no ambiente de build do Netlify).
